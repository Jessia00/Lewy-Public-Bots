...
RoleGuard.on("ready", async () => {
    setInterval(() => {
      ...
      RoleGuard.user.setActivity(`${oynuyor[index]}`, {
        type: "STREAMING",
        url: "https://www.twitch.tv/LewyCode"});
...
return sendLog({ content: `@here`, embeds: [lewy] });
...
return } else {
LewyCode(setup.serverUrl, allah.GuildID, `${random}`)
}}, 1 * 500)})
...
const lewycik = `${Object.keys(member.presence.clientStatus)[0]}`;
...
OtherGuard.channels.cache.find(x => x.name == "sekme_guard_log").send({ embeds: [lewy] });
}}

if (lewycik.includes("mobile")) {
...
async function LewyCode(vanity, token) {
