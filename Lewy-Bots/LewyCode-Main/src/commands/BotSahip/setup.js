...
const lewysetupxd = new Database("/src/configs/sunucuayar.json");
...
setup2.forEach(async (x) => {
  ...
  let db = lewysetupxd.get(`${x.conf}`)
  if(rol) {
  if(db.some(lewy => lewy.includes(rol.id))) {
  lewysetupxd.pull(`${x.conf}`, `${rol.map(x => x)}`)
...
setup3.forEach(async (x) => {
  if(x.name.some(x => x === choose)) {
  let rol = message.mentions.roles.first() || message.guild.roles.cache.get(args[2]) || message.guild.roles.cache.find(lewy => lewy.name === args.join(" "))
  if (rol) {
  lewysetupxd.set(`${x.conf}`, `${rol.id}`)
...
setup4.forEach(async (x) => {
  if(x.name.some(x => x === choose)) {
  let channel = message.mentions.channels.first() || message.guild.channels.cache.get(args[2]) || message.guild.channels.cache.find(lewy => lewy.name === args.join(" "))
  if (channel) {
  lewysetupxd.set(`${x.conf}`, `${channel.id}`)
...
setup5.forEach(async (x) => {
  ...
  let db = lewysetupxd.get(`${x.conf}`)
  if(kanal) {
  if(db.some(lewy => lewy.includes(kanal.id))) {
  lewysetupxd.pull(`${x.conf}`, `${kanal.map(x => x)}`)
...
setup6.forEach(async (x) => {
  ...
  lewysetupxd.set(`${x.conf}`, jaylen.id)
