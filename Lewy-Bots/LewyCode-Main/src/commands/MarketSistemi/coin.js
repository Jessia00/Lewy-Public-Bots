...
if (!dolarData || dolarData && !dolarData.dolar) return await message.reply({ content: `Komutu kullanabilmek için coine ihtiyacınız var. Günlük coininizi almadıysanız ${kirmiziok} \` !daily \``})
  ...
  const applyText = (canvas, text) => {
    ...
    const background = await Canvas.loadImage('https://media.discordapp.net/attachments/938786568175513660/990564563441307698/card-lewy.png?width=291&height=176');
 ...
 ctx.save();
  ...
  const attachment = new MessageAttachment(canvas.toBuffer(), 'lewy.png');
