...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.reply({ content:`\`${prefix}kayıt <@Lewy/ID> <Isim> <Yaş>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
   ...
   const tagModedata = await regstats.findOne({ guildID: message.guild.id })
    ...
    const row = new MessageActionRow()
		.addComponents(
    new MessageButton().setCustomId("MAN").setLabel("Erkek").setStyle("PRIMARY").setEmoji("916010225289560074"),
    new MessageButton().setCustomId("WOMAN").setLabel("Kadın").setStyle("SUCCESS").setEmoji("916010235200679996"),
    new MessageButton().setCustomId("İPTAL").setLabel("İptal").setStyle("DANGER").setEmoji("920412153712889877"),
	);
